 ...
 */
function getBugsForExec(&$db,&$bug_interface,$execution_id,$raw = null)
{
  ...
  if( is_object($bug_interface) )
  {
    ...
    {   
      ...
      foreach($map as $elem)
      {
        ...
        {
          $issue = $bug_interface->getIssue($elem['bug_id']);
...
if($info['issue_tracker_enabled'])
{
    ...
    $itsxxx = $it_mgr->getInterfaceObject($args->tproject_id);
    unset($it_mgr);
} 
     ...
     $args->do_action == 'result')
{
  ...
  {
    buildSpreadsheetData($db,$args,$gui,$execStatus,$labels, $itsxxx);
  }
...
//   $dataHeader[] = $lbl['assigned_to'];
  ...
  $dataHeader[] = $lbl['redmine_tracker'];
  ...
  unlink($tmpfname);
  $f2d = 'resultsTCFlat_with_bugs_'. $gui->tproject_name . '_' . $gui->tplan_name . $settings[$xlsType]['ext'];
 ...
 *
 */
function buildSpreadsheetData(&$db,&$args,&$gui,&$exec,$labels, &$bugInterface)
{
...
*/
  ...
  for($ix=0; $ix < $loop2do; $ix++)
  {
      ...
      {
          $bugs = getBugsForExec($db, $bugInterface, $metrics[$ix]['executions_id']);
          if (empty($bugs))
          {
              $bugs = array(0 => null);
          }
